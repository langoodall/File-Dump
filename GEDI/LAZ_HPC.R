...
gediMacthed <- read_csv("/share/tcsi/lagoodal/GEDI/Storage/gediMatched.csv")
fortypBWCA_UTM <- rast("/share/tcsi/lagoodal/GEDI/Storage/fortypBWCA_UTM.tif") %>% project(., "epsg:26915", method = "near", res = c(30,30))
treeMN <- fread("/share/tcsi/lagoodal/GEDI/Storage/treeMN.csv")
...
gediDf <- gediMatched %>% filter(coded %in% unique(LAS@data$code))
gediCodes <- uinique(gediDf$code)
