...
huc12StandsRast <- rast(huc12Stands[,c(1,2,5)], extent = ext(piedmontOwnership), crs = crs(piedmontOwnership), type = "xyz")

ext(piedmontOwnership) == ext(huc12Stands)


x <- as_spatraster(huc12Stands[,c(1,2,5)], xycols = c(1,2), crs = crs(piedmontOwnership))
...
test <- rast("C:/Users/lagoodal/Desktop/All Raster Layers/standsRaster/huc12Stands.tif")  %>% crop(., clippedPiedmont.shp) %>% mask(., clippedPiedmont.shp)
