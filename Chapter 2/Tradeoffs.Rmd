...
AWBDf %>%
  ...
  summarise(AWB_ = mean(AWB),
            sd = sd(AWB, na.rm = TRUE),
            n = n(),
            se = sd / sqrt(n)) %>%
  ggplot(., aes(x = Prescription, y = AWB_, fill = Prescription)) +
  geom_bar(stat = "identity") +
  geom_errorbar(aes(x = Prescription, ymin = AWB_-se, ymax = AWB_+se), width = 0.25) +
  theme_classic() +
  facet_grid(RCP ~ Model)
...
Tradeoff_TotalHarvest_TotalC)
  ...
  # select(-which(grepl("Harvest", names(.))))
...
# Tradeoff cumulative totals by Prescription, Model and RCP
tradeoffDf %>%
  mutate(Model = sub("_[^_]+$", "", Model)) %>%
  pivot_longer(cols = starts_with("Tradeoff"),
               names_to = "Tradeoff_Type",
  ...
  # filter(RCP == "RCP45") %>%
  ggplot(., aes(x = Var1, y = Value, fill = RCP)) +
...
#                values_to = "Value") %>%
#   separate(Tradeoff_Type, into = c("Prefix", "Var1", "Var2"), sep = "_") %>%
#   group_by(RCP, Model, Prescription, Var1, Var2) %>%
#   summarise(Value = sum(Value)) %>%
#   filter(RCP == "RCP45") %>%
...
tradeoffDf %>%
  ...
  pivot_longer(cols = starts_with("Tradeoff"),
               ...
               values_to = "Value") %>%
  separate(Tradeoff_Type, into = c("Prefix", "Var1", "Var2"), sep = "_") %>%
  # filter(Var1 != "Structure" & Var2 != "Structure") %>%
...
successionLogAllDf[,c(1,2,5,6,7,8,9,62:64)] %>%
  ...
  summarise(NEEC = mean(NEEC),SOMTC = mean(SOMTC),AGB = mean(AGB),Structure_H = mean(Structure_H),H = mean(H),Total_C = mean(Total_C),Total_Harvest = sum(Total_Harvest),Fruit_Trees = as.numeric(mean(Fruit_Trees)),Culture_Trees = mean(Culture_Trees),AWB = mean(AWB),ANPP = mean(ANPP)) %>%
  pivot_longer(cols = "NEEC":"ANPP", names_to = "Variable", values_to = "Value") %>%
  ggplot(., aes(x = Prescription, y = Value, fill = Model)) +
  geom_bar(stat = "identity", position = "dodge") +
  theme_classic() +
...
successionLogAllDf[,c(1,2,5,6,7,8,9,62:64)] %>%
  ...
  summarise(H = mean(H)) %>%
  ggplot(., aes(x = Prescription, y = H, fill = Prescription)) +
  geom_bar(stat = "identity") +
  theme_classic() +
  facet_grid(~RCP)
...
summary(aov(NEEC ~ Prescription * Model * RCP, data = x))
names(x)
...
tradeoffDf_long %>%
  ...
  ggplot(., aes(x = Time, y = total, fill = RCP)) +
  geom_bar(stat = "identity", position = "dodge") +
  theme_classic() +
  facet_grid(Prescription ~ Model)
...
tradeoffDf_long %>%
  ...
  group_by(RCP, Model, Prescription) %>%
  summarise(total = sum(Value) / 2) %>%
  arrange(Prescription, RCP, Model)
...
ggplot(data, aes(x = Prescription, y = mean, fill = Model)) +
...
aov.1 <- aov(Value ~ Prescription * Model * RCP, data = tradeoffDf_long)
summary(aov.1)


data <- tradeoffDf_long %>%
  select(Time, RCP, Model, Prescription, Value, Var1, Var2) %>%
  group_by(Prescription, Model, RCP, Var1, Var2) %>%
  summarise(total = sum(Value) / 2, mean = mean(Value), sd = sd(Value), n = n(), se = sd / sqrt(n)) %>%
  arrange(desc(total))

ggplot(data, aes(x = Prescription, y = mean, fill = Model)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
  # geom_errorbar(aes(ymin = mean - se, ymax = mean + se), width = 0.5, position = position_dodge(width = 0.9)) +
  theme_classic() +
  facet_grid(RCP ~ Var1)


ggplot(data, aes(x = Prescription, y = total, fill = Model)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
  # geom_errorbar(aes(ymin = mean - sd, ymax = mean + sd), width = 0.5, position = position_dodge(width = 0.9)) +
  theme_classic() +
  facet_grid(RCP ~ Var1)
...
for (i in unique(tradeoffDf_long$Time)) {
  x <- ggplot(tradeoffDf_long %>% filter(RCP == "RCP85" & Time == i,
                                  Value >= 0.5), aes(x = x, y = y, fill = Value)) +
  geom_tile() +
  scale_fill_gradient(low = "#a8dadc", high = "#1d3557", name = "Tradeoff Value") +
  labs(title = "Heatmap of Tradeoffs", x = "X", y = "Y") +
...
# Step 2: Compute Correlation for Each Model ~ Prescription Combination
cor_data <- tradeoffCorr %>%
  ...
  map_df(function(df) {
    ...
    df_wide <- df %>%
      ...
      select(-Var1)  # Drop Var1 as it's already in the column names
    
    # Ensure numeric columns only
    df_wide <- df_wide %>%
      select(where(is.numeric))  # Remove non-numeric columns
    
    # Check if we have more than one numeric column
    if (ncol(df_wide) > 1) {  
      cor_matrix <- cor(df_wide, use = "pairwise.complete.obs")  # Compute correlation
      cor_long <- as.data.frame(as.table(cor_matrix)) %>%
        rename(Var1 = Var1, Var2 = Var2, Correlation = Freq) %>%
        mutate(Model = model, Prescription = prescription, RCP = rcp)  
    } else {
      cor_long <- tibble(Var1 = NA, Var2 = NA, Correlation = NA, Model = model, Prescription = prescription)
    }
    
    return(cor_long)
  })

# Step 3: Merge Correlation Values with Tradeoff Data
plot_data <- tradeoffCorr %>%
  left_join(cor_data, by = c("Model", "RCP", "Prescription", "Var1", "Var2"))

ggplot(plot_data %>%
         filter(Model == "MIROC-ESM-CHEM" & RCP == "RCP85" & Prescription == "Resistance") %>%
         filter(Correlation <= 0.9 & Correlation >= -0.9), aes(x = Var1, y = Var2, fill = Correlation)) +
  geom_tile() +
  geom_text(aes(label = round(Correlation, 2)), color = "white", size = 4, na.rm = TRUE) +  # Overlay correlation
...
ggplot(plot_data %>%
         filter(Model == "MRI-CGCM3" & RCP == "RCP45" & Prescription == "Transition") %>%
         filter(Correlation <= 0.9 & Correlation >= -0.9), aes(x = Var1, y = Var2, fill = Correlation)) +
  geom_tile() +
  geom_text(aes(label = round(Correlation, 2)), color = "white", size = 4, na.rm = TRUE) +  # Overlay correlation
  scale_fill_gradient2(low = "#d73027", mid = "#ffffbf", high = "#1a9850", 
                       midpoint = 0, name = "Correlation") +  
  labs(title = "Most Tradeoffs", x = "X", y = "Y") +
  theme_classic() +
  facet_grid(Prescription~Model) +
  theme(legend.position = "bottom", legend.key.size = unit(3, "line"))
...
plot_data_filtered <- plot_data %>%
  filter(
    (Model == "MIROC-ESM-CHEM" & RCP == "RCP85" & Prescription == "Resistance") |
    (Model == "MRI-CGCM3" & RCP == "RCP45" & Prescription == "Transition")
  ) %>%
  filter(Correlation <= 0.9 & Correlation >= -0.9) %>%
  mutate(Tradeoff = case_when(
    Model == "MIROC-ESM-CHEM" ~ "Least Tradeoffs",
    Model == "MRI-CGCM3" ~ "Most Tradeoffs"
  ))
...
# Cumulative trade-off score by model, RCP and prescription
plot_data %>%
  filter(Correlation <= 0.9 & Correlation >= -0.9) %>%
  select(-Value) %>%
  left_join(tradeoffDf_long, by = c("Model", "RCP", "Prescription", "Var1", "Var2")) %>%
  group_by(RCP, Model, Prescription) %>%
  summarise(total = sum(Value) / 2) %>%
  ...
  ggplot(., aes(x = Model, y = total, fill = Model)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = round(total, 1)), vjust = -0.2, color = "black") +
  theme_classic() +
  facet_grid(RCP ~ Prescription) +
  scale_y_continuous(limits = c(0,105), name = "Cumulative Tradeoff Score") +
  theme(axis.ticks.x = element_blank(), axis.text.x = element_blank()) +
  scale_fill_manual(values = c(
      "BNU-ESM" = "#984ea3",
      "MIROC-ESM-CHEM" = "#ffd92f",
      "MRI-CGCM3" = "#ffb74d",
      "CCSM4" = "#377eb8"
    ),
    name = "Climatic Model")

ggsave(filename = "HighLow_Trade_Offs.png",
...
# Cumulative trade-off score by model and RCP
plot_data %>%
  filter(Correlation <= 0.9 & Correlation >= -0.9) %>%
  select(-Value) %>%
  left_join(tradeoffDf_long, by = c("Model", "RCP", "Prescription", "Var1", "Var2")) %>%
...
# Cumulative trade-off score by RCP and prescription
plot_data %>%
  filter(Correlation <= 0.9 & Correlation >= -0.9) %>%
  select(-Value) %>%
  left_join(tradeoffDf_long, by = c("Model", "RCP", "Prescription", "Var1", "Var2")) %>%
  group_by(RCP, Prescription) %>%
  summarise(total = sum(Value) / 2) %>%
  mutate(Prescription = factor(Prescription, levels = c("Resistance", "Resilience", "Transition"))) %>%
  ggplot(., aes(x = Prescription, y = total, fill = Prescription)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = round(total, 1)), vjust = -0.2, color = "black") +
  theme_classic() +
  facet_grid(~ RCP) +
...
# Cumulative trade-off score by model and prescription
plot_data %>%
  filter(Correlation <= 0.9 & Correlation >= -0.9) %>%
  select(-Value) %>%
  left_join(tradeoffDf_long, by = c("Model", "RCP", "Prescription", "Var1", "Var2")) %>%
  group_by(Model, Prescription) %>%
  summarise(total = sum(Value) / 2) %>%
  mutate(Prescription = factor(Prescription, levels = c("Resistance", "Resilience", "Transition"))) %>%
  ggplot(., aes(x = Prescription, y = total, fill = Prescription)) +
  geom_bar(stat = "identity") +
...
# Step 4: Generate the Heatmap with Correlation Values
ggplot(plot_data, aes(x = Var1, y = Var2, fill = Correlation)) +
  geom_tile() +
  geom_text(aes(label = round(Correlation, 2)), color = "white", size = 4, na.rm = TRUE) +  # Overlay correlation
  scale_fill_gradient2(low = "#d73027", mid = "#ffffbf", high = "#1a9850", 
                       midpoint = 0, name = "Correlation") +  
  labs(title = "Heatmap of Tradeoff Correlations (Per Model ~ Prescription)", x = "X", y = "Y") +
  theme_classic() +
  facet_grid(Prescription ~ Model)

ggplot(plot_data %>% filter(Correlation < 0.8 & Correlation > 0.7), aes(x = Var1, y = Var2, fill = Correlation)) +
  geom_tile() +
  geom_text(aes(label = round(Correlation, 2)), color = "white", size = 4, na.rm = TRUE) +  # Overlay correlation
  scale_fill_gradient2(low = "#d73027", mid = "#ffffbf", high = "#1a9850", 
                       midpoint = 0, name = "Correlation") +  
...
plot_data %>% filter(Correlation < 0.8 & Correlation > 0.7) %>%
  group_by(Model, Prescription, Var1) %>%
  summarise(n = n()) %>%
...
plot_data %>%
  filter(Correlation > -0.8 & Correlation < -0.7) %>%
  group_by(Prescription) %>%
  summarise(n = n()) %>%
  ggplot(., aes(x = Prescription, y = n)) +
  geom_bar(stat = "identity") +
  theme_classic() +
...
plot_data %>%
  ...
  ggplot(., aes(x = Prescription, y = Correlation)) +
  geom_bar(stat = "identity") +
  theme_classic()

aov.1 <- aov(Correlation ~ Prescription, data = plot_data)
summary(aov.1)
TukeyHSD(aov.1)



plot(rast("C:/Users/lagoodal/Documents/TEST HPC OUTPUTS/Clipped_Piedmont/finalTreeMap.tif"))
```
...
x <- corrDf %>%
  ...
  select(-Time, -Prescription, -Model, -RCP)
